...
Test(fstests, test_13){
   ...
   cr_expect(ilink(1, name, &ip, f) == 0, "Should be able to add direnty to root inode");
...
Test(fstests, test_14){
    ...
    cr_expect(iwrite(&ip, data, strlen((char *)data) + 1, 0, f) == (int)(strlen((char *)data) + 1), "Write should return length of hello world!");
}
...
Test(fstests, test_17) {
    ...
    iread(&ip, data, strlen("Hello world!") + 1, 0, f);
